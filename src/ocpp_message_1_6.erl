...
-export_type(['AuthorizeRequest'/0,
              'AuthorizeResponse'/0,
...
-type 'AuthorizeResponse'() :: #{idTagInfo :=
                                     #{status := string(), parentIdTag => string(),
                                       expiryDate => calendar:datetime()}}.

-type 'BootNotificationRequest'() :: #{meterType => string(),
                                       meterSerialNumber => string(), imsi => string(),
                                       iccid => string(), firmwareVersion => string(),
                                       chargePointVendor := string(),
                                       chargePointSerialNumber => string(),
                                       chargePointModel := string(),
                                       chargeBoxSerialNumber => string()}.

-type 'BootNotificationResponse'() :: #{status := string(), interval := integer(),
...
-type 'ClearChargingProfileRequest'() :: #{stackLevel => integer(), id => integer(),
                                           connectorId => integer(),
...
-type 'GetCompositeScheduleRequest'() :: #{duration := integer(),
                                           connectorId := integer(),
                                           chargingRateUnit => string()}.

-type 'GetCompositeScheduleResponse'() :: #{status := string(),
...
-type 'GetConfigurationResponse'() :: #{unknownKey => [string()],
                                        configurationKey =>
                                            [#{value => string(), readonly := boolean(),
                                               key := string()}]}.

-type 'GetDiagnosticsRequest'() :: #{stopTime => calendar:datetime(),
                                     startTime => calendar:datetime(),
                                     retryInterval => integer(), retries => integer(),
                                     location := string()}.
...
-type 'HeartbeatResponse'() :: #{currentTime := calendar:datetime()}.

-type 'MeterValuesRequest'() :: #{transactionId => integer(),
                                  meterValue :=
...
-type 'RemoteStartTransactionRequest'() :: #{idTag := string(),
                                             connectorId => integer(),
                                             chargingProfile =>
...
-type 'ReserveNowRequest'() :: #{reservationId := integer(), parentIdTag => string(),
                                 idTag := string(), expiryDate := calendar:datetime(),
                                 connectorId := integer()}.
...
-type 'SendLocalListRequest'() :: #{updateType := string(),
...
-type 'SetChargingProfileRequest'() :: #{csChargingProfiles :=
                                             #{validTo => calendar:datetime(),
                                               validFrom => calendar:datetime(),
                                               transactionId => integer(),
                                               stackLevel := integer(),
...
-type 'StartTransactionRequest'() :: #{timestamp := calendar:datetime(),
                                       reservationId => integer(),
                                       meterStart := integer(), idTag := string(),
                                       connectorId := integer()}.

-type 'StartTransactionResponse'() :: #{transactionId := integer(),
                                        idTagInfo :=
                                            #{status := string(),
                                              parentIdTag => string(),
                                              expiryDate => calendar:datetime()}}.

-type 'StatusNotificationRequest'() :: #{vendorId => string(),
...
-type 'StopTransactionRequest'() :: #{transactionId := integer(),
                                      transactionData =>
                                          [#{timestamp := calendar:datetime(),
                                             sampledValue :=
                                                 [#{value := string(), unit => string(),
                                                    ...
                                                    context => string()}]}],
                                      timestamp := calendar:datetime(),
                                      reason => string(), meterStop := integer(),
                                      idTag => string()}.

-type 'StopTransactionResponse'() :: #{idTagInfo =>
                                           #{status := string(),
                                             parentIdTag => string(),
                                             expiryDate => calendar:datetime()}}.

-type 'TriggerMessageRequest'() :: #{requestedMessage := string(),
                                     connectorId => integer()}.
...
-type 'UpdateFirmwareRequest'() :: #{retryInterval => integer(),
                                     retrieveDate := calendar:datetime(),
                                     retries => integer(), location := string()}.
...
-define(BootNotificationResponse,
        {#{status => cmap:enum_(['Accepted', 'Pending', 'Rejected']),
           interval => cmap:integer_(#{}), currentTime => fun cmap:datetime/1},
...
-define(ClearChargingProfileRequest,
        {#{stackLevel => cmap:integer_(#{}), id => cmap:integer_(#{}),
           connectorId => cmap:integer_(#{}),
...
-define(ClearChargingProfileResponse,
        {#{status => cmap:enum_(['Accepted', 'Unknown'])},
         [{required, [status]}, {extra_keys, false}]}).

-define(DataTransferRequest,
        {#{vendorId => cmap:string_(#{max_length => 255}),
           messageId => cmap:string_(#{max_length => 50}), data => cmap:string_(#{})},
...
-define(DiagnosticsStatusNotificationResponse,
        {#{}, [{required, []}, {extra_keys, false}]}).
...
-define(GetCompositeScheduleRequest,
        {#{duration => cmap:integer_(#{}), connectorId => cmap:integer_(#{}),
           chargingRateUnit => cmap:enum_(['A', 'W'])},
         [{required, [connectorId, duration]}, {extra_keys, false}]}).

-define(GetCompositeScheduleResponse,
        {#{status => cmap:enum_(['Accepted', 'Rejected']),
           scheduleStart => fun cmap:datetime/1, connectorId => cmap:integer_(#{}),
           chargingSchedule =>
               fun (Input) ->
                       cmap:new(#{startSchedule => fun cmap:datetime/1,
                                  minChargingRate => fun cmap:number/1,
                                  duration => cmap:integer_(#{}),
                                  chargingSchedulePeriod =>
                                      cmap:list_(#{items =>
                                                       fun (Input) ->
                                                               cmap:new(#{startPeriod =>
                                                                              cmap:integer_(#{}),
                                                                          numberPhases
                                                                              =>
                                                                              cmap:integer_(#{}),
...
-define(GetDiagnosticsRequest,
        {#{stopTime => fun cmap:datetime/1, startTime => fun cmap:datetime/1,
...
-define(GetLocalListVersionResponse,
        {#{listVersion => cmap:integer_(#{})},
         [{required, [listVersion]}, {extra_keys, false}]}).
...
-define(MeterValuesRequest,
        {#{transactionId => cmap:integer_(#{}),
           meterValue =>
               cmap:list_(#{items =>
                                fun (Input) ->
                                        cmap:new(#{timestamp => fun cmap:datetime/1,
                                                   sampledValue =>
                                                       cmap:list_(#{items =>
                                                                        fun (Input) ->
                                                                                cmap:new(#{value
                                                                                           ...
                                                                                           measurand
                                                                                               =>
                                                                                               cmap:enum_(['Energy.Active.Export.Register',
...
-define(RemoteStartTransactionRequest,
        {#{idTag => cmap:string_(#{max_length => 20}),
           ...
           chargingProfile =>
               fun (Input) ->
                       cmap:new(#{validTo => fun cmap:datetime/1,
                                  ...
                                  chargingProfilePurpose =>
                                      cmap:enum_(['ChargePointMaxProfile',
                                                  'TxDefaultProfile',
                                                  'TxProfile']),
                                  chargingProfileKind =>
...
-define(ReserveNowRequest,
        {#{reservationId => cmap:integer_(#{}),
           parentIdTag => cmap:string_(#{max_length => 20}),
           idTag => cmap:string_(#{max_length => 20}),
           expiryDate => fun cmap:datetime/1, connectorId => cmap:integer_(#{})},
...
-define(StartTransactionRequest,
        {#{timestamp => fun cmap:datetime/1, reservationId => cmap:integer_(#{}),
           meterStart => cmap:integer_(#{}), idTag => cmap:string_(#{max_length => 20}),
           connectorId => cmap:integer_(#{})},
...
-define(StatusNotificationRequest,
        {#{vendorId => cmap:string_(#{max_length => 255}),
           ...
           errorCode =>
               cmap:enum_(['ConnectorLockFailure',
        ...
        {#{transactionId => cmap:integer_(#{}),
           ...
           reason =>
               cmap:enum_(['EmergencyStop',
                           ...
                           'UnlockCommand',
                           'DeAuthorized']),
           meterStop => cmap:integer_(#{}), idTag => cmap:string_(#{max_length => 20})},
...
-define(UnlockConnectorRequest,
        {#{connectorId => cmap:integer_(#{})},
         [{required, [connectorId]}, {extra_keys, false}]}).
...
-define(UpdateFirmwareRequest,
        {#{retryInterval => cmap:integer_(#{}), retrieveDate => fun cmap:datetime/1,
           retries => cmap:integer_(#{}), location => cmap:string_(#{})},
...
-spec
     change_configuration_request('ChangeConfigurationRequest'()) -> message('ChangeConfigurationRequest'()).
change_configuration_request(ChangeConfigurationRequest) ->
    {Spec, Opts} = (?ChangeConfigurationRequest),
    {?MODULE,
     'ChangeConfigurationRequest',
     cmap:new(Spec, ChangeConfigurationRequest, Opts)}.

-spec
     change_configuration_response('ChangeConfigurationResponse'()) -> message('ChangeConfigurationResponse'()).
change_configuration_response(ChangeConfigurationResponse) ->
    {Spec, Opts} = (?ChangeConfigurationResponse),
    {?MODULE,
     'ChangeConfigurationResponse',
     cmap:new(Spec, ChangeConfigurationResponse, Opts)}.
...
-spec
     diagnostics_status_notification_request('DiagnosticsStatusNotificationRequest'()) -> message('DiagnosticsStatusNotificationRequest'()).
...
-spec
     diagnostics_status_notification_response('DiagnosticsStatusNotificationResponse'()) -> message('DiagnosticsStatusNotificationResponse'()).
...
-spec
     get_configuration_request('GetConfigurationRequest'()) -> message('GetConfigurationRequest'()).
...
-spec
     get_configuration_response('GetConfigurationResponse'()) -> message('GetConfigurationResponse'()).
get_configuration_response(GetConfigurationResponse) ->
    {Spec, Opts} = (?GetConfigurationResponse),
    {?MODULE,
     'GetConfigurationResponse',
     cmap:new(Spec, GetConfigurationResponse, Opts)}.

-spec
     get_diagnostics_request('GetDiagnosticsRequest'()) -> message('GetDiagnosticsRequest'()).
...
-spec
     get_diagnostics_response('GetDiagnosticsResponse'()) -> message('GetDiagnosticsResponse'()).
...
-spec
     remote_start_transaction_request('RemoteStartTransactionRequest'()) -> message('RemoteStartTransactionRequest'()).
remote_start_transaction_request(RemoteStartTransactionRequest) ->
    {Spec, Opts} = (?RemoteStartTransactionRequest),
    {?MODULE,
     'RemoteStartTransactionRequest',
     cmap:new(Spec, RemoteStartTransactionRequest, Opts)}.

-spec
     remote_start_transaction_response('RemoteStartTransactionResponse'()) -> message('RemoteStartTransactionResponse'()).
...
-spec
     remote_stop_transaction_request('RemoteStopTransactionRequest'()) -> message('RemoteStopTransactionRequest'()).
remote_stop_transaction_request(RemoteStopTransactionRequest) ->
    {Spec, Opts} = (?RemoteStopTransactionRequest),
    {?MODULE,
     'RemoteStopTransactionRequest',
     cmap:new(Spec, RemoteStopTransactionRequest, Opts)}.

-spec
     remote_stop_transaction_response('RemoteStopTransactionResponse'()) -> message('RemoteStopTransactionResponse'()).
remote_stop_transaction_response(RemoteStopTransactionResponse) ->
    {Spec, Opts} = (?RemoteStopTransactionResponse),
    {?MODULE,
     'RemoteStopTransactionResponse',
     cmap:new(Spec, RemoteStopTransactionResponse, Opts)}.
...
-spec
     start_transaction_request('StartTransactionRequest'()) -> message('StartTransactionRequest'()).
...
-spec
     start_transaction_response('StartTransactionResponse'()) -> message('StartTransactionResponse'()).
start_transaction_response(StartTransactionResponse) ->
    {Spec, Opts} = (?StartTransactionResponse),
    {?MODULE,
     'StartTransactionResponse',
     cmap:new(Spec, StartTransactionResponse, Opts)}.
...
-spec
     stop_transaction_request('StopTransactionRequest'()) -> message('StopTransactionRequest'()).
...
-spec
     stop_transaction_response('StopTransactionResponse'()) -> message('StopTransactionResponse'()).
