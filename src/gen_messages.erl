...
gen(Args = #{schemadir := Path, module := Module}) ->
        ...
        [
            {ctors, [
                erl_syntax:map_field_assoc(
                    erl_syntax:binary([
                        erl_syntax:binary_field(erl_syntax:string(atom_to_list(MType)))
                    ]),
                    erl_syntax:atom(snake_case(atom_to_list(MType)))
                )
             || MType <- TypeNames,
                lists:member(
                    list_to_atom(snake_case(atom_to_list(MType))),
                    ExportNames
                )
            ]}
        ]
    ...
    Decode = ?Q(
        [
            ...
            "try maps:get(PayloadType, _@decoders) of Decoder -> decode_with(Decoder, PayloadType, Payload) "
            "catch error:badkey -> {error, unknown_message_type} end.",
        ...
        [
            ...
            {decoders, erl_syntax:macro(erl_syntax:variable("DECODERS"))}
        ]
    ),
    file:write_file(
        ...
        string:join(
            [
                ...
                Payload
                | TypeFormStrs
            ] ++ SpecFormStrs ++ [erl_prettypr:format(Decoders)] ++ CtorStrs ++
                [
                    string:join(
                        [
