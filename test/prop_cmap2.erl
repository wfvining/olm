    ...
    ?FORALL(
        ...
        begin
            ...
            Res =
                if
                    Valid ->
                        {ok, #{key => X}} =:= cmap2:new(#{key => X}, Spec);
                    Valid =:= badtype ->
                        make_error(type_error, [key], X, boolean) =:=
                            cmap2:new(#{key => X}, Spec)
                end,
            collect(Valid, Res)
        end
    ).
                ...
                if
                    Valid =:= badtype ->
                        make_error(type_error, [key], I, {integer, Constraint}) =:=
                            cmap2:new(#{key => I}, OSpec);
                    Valid ->
                        {ok, #{key => I}} =:= cmap2:new(#{key => I}, OSpec);
                    not Valid ->
                ...
                if
                    Valid =:= badtype ->
                        make_error(type_error, [key], Str, {string, Constraint}) =:=
                            cmap2:new(#{key => Str}, OSpec);
                    Valid ->
                        {ok, #{key => Str}} =:= cmap2:new(#{key => Str}, OSpec);
                    not Valid ->
                ...
                if
                    Valid =:= badtype ->
                        make_error(type_error, [key], Value, {enum, Values}) =:=
                            cmap2:new(#{key => Value}, OSpec);
                    Valid, is_binary(Value) ->
                        {ok, #{key => binary_to_existing_atom(Value)}} =:=
                            cmap2:new(#{key => Value}, OSpec);
                    Valid, is_atom(Value) ->
                        {ok, #{key => Value}} =:= cmap2:new(#{key => Value}, OSpec);
                    not Valid ->
        ...
        begin
            ...
            case Valid of
                ...
                {illegal_properties, Keys} ->
                    collect(
                        ...
                        {error, {property_error, {illegal_properties, Keys}}} =:=
                            ...
                            cmap2:new(Object, Constraints)
                    );
                {missing_properties, Missing} ->
                    collect(
                        missing_properties,
                        case cmap2:new(Object, Constraints) of
                ...
                Errors when is_list(Errors) ->
                    Res = cmap2:new(#{key => Object}, OSpec),
                    collect(
                        multierror,
                        lists:any(
                            fun
...
load_value(X, Spec) ->
    ...
    {ok, #{key := X1}} = cmap2:new(#{key => X}, #{properties => #{key => Spec}}),
    X1.
